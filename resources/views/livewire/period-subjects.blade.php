            ...
            @foreach($assignedSubjects as $subject)
                <tr>
                    <td class="px-6 py-4 whitespace-nowrap">
                        <select
                            wire:change="updateSubjectStatus({{ $subject->id }}, $event.target.value)"
                            class="block rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500 p-2 border text-sm"
                        >
                            @foreach($statusOptions as $status)
                                <option
                                    value="{{ $status->value }}"
                                    @if(isset($subjectStatuses[$subject->id]) && $subjectStatuses[$subject->id] == $status->value)
                                        selected
                                    @endif
                                >
                                    {{ \App\Enums\StudentEnrollmentStatusEnum::getNameByValue($status->value) }}
                                </option>
                            @endforeach
                        </select>
                    </td>
